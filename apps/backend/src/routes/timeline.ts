...
const hiddenTimelineTypes = ["parking-space-groups", "maintenance-tickets", "maintenance_tickets", "inspections", "attachments"];
  ...
  router.get(
    ...
    asyncHandler((req, res) => {
      ...
      if (req.query.module) {
        ...
        where.push("(e.entity_type = ? OR e.entity_type = ?)");
        params.push(module, module.replace(/-/g, "_"));
      }
      if (req.query.status) {
        where.push("e.status = ?");
      ...
      const rows = db
        .prepare(
          `SELECT
             e.id,
             e.structure_id,
             REPLACE(e.entity_type, '_', '-') AS entity_type,
             e.entity_id,
             e.event_type,
             e.event_date,
             e.title,
